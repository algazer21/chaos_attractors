...
Alan García Zermeño
5/12/22
Resuelve el sistema dinámico de Rabinovich usando Runge Kutta 4
***********************************************************/
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
...
#define GAMMA 0.1
#define ALPHA 0.05 //0.14

void createvalues(size_t,double*,double*,double*);
void RKutta4(size_t,double,double*,double*,double*);
double dx(double t, double x, double y, double z){ return(y*(z-1.0 + x*x) + GAMMA*x); }
double dy(double t, double x, double y, double z){ return(x*(3.0*z + 1.0 - x*x) + GAMMA*y); }
double dz(double t, double x, double y, double z){ return(-2.0*z*(ALPHA + x*y)); }


int main(){
	unsigned int n = 100000;
	double dt = 0.001;    //dt = 0.5

	double *x = (double *)malloc(n*sizeof(double));
   double *y = (double *)malloc(n*sizeof(double));
   double *z = (double *)malloc(n*sizeof(double));

	x[0] = 0.1; y[0] = -0.1; z[0] = 0.1;   //x[0] = -1.0; y[0] = 0.0; z[0] = 0.5;
	RKutta4(n,dt,x,y,z);
